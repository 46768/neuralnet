...
void nn_gradient_descent(float* buffer, float* gradient, size_t gradient_size, void* self) {
...
void nn_gradient_descent_finalize(float* buffer, float* gradient, size_t gradient_size, void* self) {
	size_t float_cnt = gradient_size / sizeof(float);
	float grad_coef = (float)1 / ((Optimizer*)(self))->batch_size;
